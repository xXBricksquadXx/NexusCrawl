...
def search_intel(keyword: str):
    ...
    for file, page, content in results:
        # Find occurrences to build a context window around the target word
        matches = [m.span() for m in re.finditer(re.escape(keyword), content, re.IGNORECASE)]
        
        print(f"📄 FILE: {file} | PAGE: {page}")
        
        # Limit to the first 2 matches per page so we don't flood the terminal
        for start, end in matches[:2]: 
            # Grab 60 characters of text on either side of the keyword
            window_start = max(0, start - 60)
            window_end = min(len(content), end + 60)
            
            snippet = content[window_start:window_end].replace('\n', ' ')
            
            # Visually highlight the keyword in the terminal
            snippet = re.sub(f"({re.escape(keyword)})", r" [ \1 ] ", snippet, flags=re.IGNORECASE)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--keyword", type=str, required=True, help="The target word or phrase to hunt for.")
