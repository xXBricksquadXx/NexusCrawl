    ...
    async def start(self):
        ...
        for url in self.spider.start_urls:
            await self.queue.put(
                Request(
                    url=url, callback=self.spider.parse, render_js=default_render_js
                )
            )
                ...
                try:
                    ...
                    if request.render_js:
                        ...
                        response = await self.retry_middleware.execute_with_retry(
                            request=request, client=client, headers=headers
                        )
                        html = response.text
                        ...
                        if isinstance(output, Request):
                            ...
                            if getattr(output, "image_url", None):
                                await self.media_pipeline.download_image(
                                    media_url=output.image_url, filename=output.title
                                )
