...
class RetryMiddleware:
    ...
    async def execute_with_retry(
        self, request, client: httpx.AsyncClient, headers: dict
    ):
        ...
        while attempts <= self.max_retries:
            try:
                ...
                print(
                    f"[RETRY] Server returned {response.status_code} for {request.url}. Retrying in {backoff_time}s... ({attempts}/{self.max_retries})"
                )
                ...
                backoff_time = 2**attempts
                print(
                    f"[NETWORK ERROR] Failed to connect to {request.url}: {e}. Retrying in {backoff_time}s..."
                )
                await asyncio.sleep(backoff_time)
