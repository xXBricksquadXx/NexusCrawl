...
# The core high-value indicators we are hunting for
DEFAULT_THREATS = [
    "fraud", "misappropriation", "shortage", "indicted", 
    "unauthorized", "deficit", "embezzlement", "subpoena",
    "noncompliance", "unallowable", "malfeasance"
]
...
def generate_matrix(target_location=None, output_format="md", context_size=400):
    ...
    # 2. THE THREAT ENGINE
    findings = []
    threat_pattern = re.compile(r'\b(' + '|'.join(DEFAULT_THREATS) + r')\b', re.IGNORECASE)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target", type=str, help="Optional: Filter by a specific county or city (e.g., 'Giles')")
    parser.add_argument("--format", type=str, choices=["md", "csv"], default="md", help="Output format: 'md' (Markdown) or 'csv'")
    parser.add_argument("--context", type=int, default=400, help="Total characters to extract around the threat (default: 400)")
    args = parser.parse_args()

    generate_matrix(target_location=args.target, output_format=args.format, context_size=args.context)